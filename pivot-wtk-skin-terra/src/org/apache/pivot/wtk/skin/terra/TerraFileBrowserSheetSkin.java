 ...
 */
public class TerraFileBrowserSheetSkin extends TerraSheetSkin implements
	...
	private static class SaveToFileFilter implements Filter<File> {
		...
		public boolean include(File file) {
			return (!file.isDirectory() || (sourceFilter != null && sourceFilter
					.include(file)));
		}
	}
	...
	public void install(Component component) {
		...
		Component content;
		try {
			content = (Component) bxmlSerializer.readObject(
					TerraFileBrowserSheetSkin.class,
					"terra_file_browser_sheet_skin.bxml"); // remove localize
		...
		fileBrowser.getFileBrowserListeners().add(
				new FileBrowserListener.Adapter() {
					@Override
					public void rootDirectoryChanged(
							FileBrowser fileBrowserArgument,
							File previousRootDirectory) {
						updatingSelection = true;

						fileBrowserSheet.setRootDirectory(fileBrowserArgument
								.getRootDirectory());
					...
					public void selectedFileAdded(
							FileBrowser fileBrowserArgument, File file) {
						if (file.isDirectory()) {
					...
					@Override
					public void selectedFileRemoved(
							FileBrowser fileBrowserArgument, File file) {
						if (file.isDirectory()) {
							selectedDirectoryCount--;
						}
					...
					public void selectedFilesChanged(
						...
						Sequence<File> selectedFiles = fileBrowserArgument
								.getSelectedFiles();
						...
						if (!fileBrowserArgument.isMultiSelect()) {
							File selectedFile = fileBrowserArgument
									.getSelectedFile();
				...
				new ComponentMouseButtonListener.Adapter() {
					...
					public boolean mouseClick(Component componentArgument,
						...
						if (count == 1) {
							...
							if (fileLocal != null && this.file != null
	...
	public Vote previewSheetClose(final Sheet sheet, final boolean result) {
		...
		if (result && !okButton.isEnabled()) {
			...
			if (result) {
				...
				case OPEN_MULTIPLE:
				case SAVE_TO: {
					fileBrowserSheet.setSelectedFiles(fileBrowser
							.getSelectedFiles());
					break;
				}
				...
				case SAVE_AS: {
					...
					if (!selectedFile.isAbsolute()
						...
						selectedFile = new File(fileBrowser.getRootDirectory(),
								fileName);
					} else {
					...
					if (selectedFile != null) {
						fileBrowserSheet.setSelectedFiles(new ArrayList<File>(
								selectedFile));
					}
					break;
				}
	...
	private void updateDisabledFileFilter() {
		...
		Filter<File> disabledFileFilter = fileBrowserSheet
