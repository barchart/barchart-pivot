...
public class TerraListButtonSkin extends ListButtonSkin {
	private WindowStateListener listViewPopupStateListener = new WindowStateListener.Adapter() {
		...
		public void windowOpened(Window window) {
			...
			if (listSize == -1) {
				...
				if (!listViewBorder.isPreferredHeightSet()) {
					ListView.ItemRenderer itemRenderer = listView
							.getItemRenderer();
					int borderHeight = itemRenderer.getPreferredHeight(-1)
							* listSize + 2;
			...
			window.setLocation(buttonLocation.x, buttonLocation.y + getHeight()
					- 1);
			...
			ApplicationContext.queueCallback(new Runnable() {
				...
				public void run() {
					...
					if (selectedIndex >= 0) {
						Bounds itemBounds = listView
								.getItemBounds(selectedIndex);
		...
		public Vote previewWindowClose(final Window window) {
			...
			if (closeTransition == null) {
				...
				vote = (closeTransition.isRunning()) ? Vote.DEFER
						: Vote.APPROVE;
			}
	...
	public int getPreferredHeight(int width) {
		...
		int preferredHeight = dataRenderer.getPreferredHeight(-1) + padding.top
				+ padding.bottom + 2;
	...
	public Dimensions getPreferredSize() {
		...
		int preferredHeight = contentSize.height + padding.top + padding.bottom
				+ 2;
	...
	public int getBaseline(int width, int height) {
		...
		int clientWidth = Math.max(width
				- (TRIGGER_WIDTH + padding.left + padding.right + 2), 0);
	...
	public void paint(Graphics2D graphics) {
		...
		if (listButton.isEnabled()) {
			...
			bevelColorLocal = (pressed || (listViewPopup.isOpen() && !listViewPopup
					.isClosing())) ? pressedBevelColor : this.bevelColor;
		...
		graphics.fill(new RoundRectangle2D.Double(0.5, 0.5, width - 1,
				height - 1, CORNER_RADIUS, CORNER_RADIUS));

		// Paint the content
		graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_OFF);

		Bounds contentBounds = new Bounds(0, 0, Math.max(width - TRIGGER_WIDTH
				- 1, 0), Math.max(height - 1, 0));
		Button.DataRenderer dataRenderer = listButton.getDataRenderer();
		dataRenderer.render(listButton.getButtonData(), listButton, false);
		dataRenderer.setSize(
				Math.max(contentBounds.width
						- (padding.left + padding.right + 2) + 1, 0),
				Math.max(contentBounds.height
						- (padding.top + padding.bottom + 2) + 1, 0));
		...
		graphics.draw(new Line2D.Double(contentBounds.x + contentBounds.width,
				0.5, contentBounds.x + contentBounds.width,
		...
		if (listButton.isFocused()) {
			...
			graphics.draw(new RoundRectangle2D.Double(2.5, 2.5, Math.max(
					contentBounds.width - 4, 0), Math.max(
					contentBounds.height - 4, 0), CORNER_RADIUS / 2,
					CORNER_RADIUS / 2));
		}
		...
		// Paint the trigger
		GeneralPath triggerIconShape = new GeneralPath(
		...
		int tx = triggerBounds.x
				+ Math.round((triggerBounds.width - triggerIconShape
						.getBounds().width) / 2f) - 1;
		int ty = triggerBounds.y
				+ Math.round((triggerBounds.height - triggerIconShape
						.getBounds().height) / 2f) - 1;
		...
		if (listButton.isRepeatable()) {
			...
			if (mouseLocation != null) {
				...
				if (triggerBounds.contains(mouseLocation)) {
					graphics.clipRect(triggerBounds.x, triggerBounds.y,
							triggerBounds.width, height);
				} else {
	...
	public final void setDisabledBackgroundColor(String disabledBackgroundColor) {
		...
		setDisabledBackgroundColor(GraphicsUtilities
				.decodeColor(disabledBackgroundColor));
	}
	...
	public final void setDisabledBorderColor(String disabledBorderColor) {
		...
		setDisabledBorderColor(GraphicsUtilities
				.decodeColor(disabledBorderColor));
	}
	...
	public void setListSelectionBackgroundColor(
			Object listSelectionBackgroundColor) {
	...
	public void setListInactiveSelectionBackgroundColor(
			Object listInactiveSelectionBackgroundColor) {
	...
	public void setListHighlightBackgroundColor(
			Object listHighlightBackgroundColor) {
