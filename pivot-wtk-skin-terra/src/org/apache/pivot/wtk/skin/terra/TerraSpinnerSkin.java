...
public class TerraSpinnerSkin extends ContainerSkin implements Spinner.Skin, SpinnerListener, SpinnerSelectionListener {
	...
	private static class AutomaticSpinner {
		 ...
		 *                If automatic spinner of any spinner is already in
		 *                progress. Only one spinner may be automatically spun
		 *                at one time
		 */
		public void start(Spinner spinnerArgument, int directionArgument) {
			...
			if (spinnerArgument.isEnableAutoSpinOnMouseDown()) {
	...
	protected class SpinnerContentSkin extends ComponentSkin {
		...
		public void paint(Graphics2D graphics) {
			...
			if (spinnerContentLocal.isFocused()) {
				BasicStroke dashStroke = new BasicStroke(1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1.0f,
				...
				graphics.draw(new Rectangle2D.Double(1, 1.5, Math.max(width - 2.5, 0), Math.max(height - 3, 0)));
			}
		}
		...
		public boolean keyTyped(Component component, char character) {
			 ...
			 * 
			 * for (int i = spinner.getSelectedIndex() + 1, n =
			 * spinnerData.getLength(); i < n; i++) { String string =
			 * itemRenderer.toString(spinnerData.get(i));
			 * 
			 * if (string != null && string.length() > 0) { char first =
			 * Character.toUpperCase(string.charAt(0));
			 * 
			 * if (first == characterUpper) { spinner.setSelectedIndex(i);
			 * consumed = true; break; } } }
			 */
	...
	public int getPreferredWidth(int height) {
		...
		preferredWidth += Math
				.max(upButton.getPreferredWidth(buttonHeight), downButton.getPreferredWidth(buttonHeight));
	...
	public void paint(Graphics2D graphics) {
		...
		graphics.setPaint(new GradientPaint(buttonX + buttonWidth / 2, 0, buttonBevelColor, buttonX + buttonWidth / 2,
				buttonHeight, buttonBackgroundColor));
		graphics.fillRect(buttonX, 0, buttonWidth, height);

		graphics.setPaint(borderColor);
		GraphicsUtilities.drawRect(graphics, 0, 0, width, height);
		GraphicsUtilities.drawLine(graphics, width - buttonWidth - 2, 0, height, Orientation.VERTICAL);
		GraphicsUtilities.drawLine(graphics, width - buttonWidth - 2, buttonHeight + 1, buttonWidth + 1,
