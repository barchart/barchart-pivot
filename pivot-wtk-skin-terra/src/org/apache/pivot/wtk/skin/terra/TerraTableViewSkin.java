 ...
 */
public class TerraTableViewSkin extends ComponentSkin implements
		TableView.Skin, TableViewListener, TableViewColumnListener,
		TableViewRowListener, TableViewSelectionListener {
	private Font font;
	private Color color;
	...
	public int getPreferredWidth(int height) {
		return getPreferredWidth((TableView) getComponent(),
		...
		for (int i = 0; i < n; i++) {
			...
			if (!column.isRelative()) {
				...
				if (columnWidth == -1) {
					...
					TableView.CellRenderer cellRenderer = column
							.getCellRenderer();

					int rowIndex = 0;
					for (Object rowData : tableData) {
						cellRenderer.render(rowData, rowIndex++, i, tableView,
								column.getName(), false, false, false);
						columnWidth = Math
								.max(cellRenderer.getPreferredWidth(-1),
										columnWidth);
					}
				}

				preferredWidth += Math.min(
						Math.max(columnWidth, column.getMinimumWidth()),
	...
	public int getPreferredHeight(int width) {
		...
		if (variableRowHeight) {
			ArrayList<Integer> columnWidthsLocal = getColumnWidths(tableView,
					width);
	...
	public int getBaseline(int width, int height) {
		...
		if (variableRowHeight) {
			...
			for (int i = 0, n = columns.getLength(); i < n; i++) {
				...
				cellRenderer.render(rowData, 0, i, tableView, column.getName(),
						false, false, false);
				baseline = Math.max(baseline, cellRenderer.getBaseline(
						columnWidthsLocal.get(i), rowHeight));
			}
			...
			for (int i = 0, n = columns.getLength(); i < n; i++) {
				TableView.Column column = columns.get(i);
				TableView.CellRenderer cellRenderer = column.getCellRenderer();
				cellRenderer.render(null, -1, i, tableView, column.getName(),
						false, false, false);
				baseline = Math.max(baseline, cellRenderer.getBaseline(
						columnWidthsLocal.get(i), rowHeight));
			}
	...
	public void layout() {
		...
		if (variableRowHeight) {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int columnIndex = 0, columnCount = columns.getLength(); columnIndex < columnCount; columnIndex++) {
					...
					cellRenderer.render(rowData, i, columnIndex, tableView,
							column.getName(), false, false, false);
		...
		if (validateSelection) {
			...
			if (selectedRanges.getLength() > 0) {
				...
				Bounds visibleSelectionBounds = tableView
	...
	private static int calculateFixedRowHeight(TableView tableView) {
		...
		for (int i = 0, n = columns.getLength(); i < n; i++) {
			...
			fixedRowHeight = Math.max(fixedRowHeight,
					cellRenderer.getPreferredHeight(-1));
		}
	...
	public void paint(Graphics2D graphics) {
		...
		if (columnSelectionColor != null) {
			...
			for (int columnIndex = 0, columnCount = columns.getLength(); columnIndex < columnCount; columnIndex++) {
				...
				SortDirection sortDirection = tableView.getSort().get(
						columnName);
		...
		for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
			...
			boolean rowHighlighted = (rowIndex == highlightIndex && tableView
					.getSelectMode() != TableView.SelectMode.NONE);
			...
			for (int columnIndex = 0, columnCount = columns.getLength(); columnIndex < columnCount; columnIndex++) {
				...
				Graphics2D rendererGraphics = (Graphics2D) graphics.create(
						columnX, rowY, columnWidth, rowHeight);

				cellRenderer.render(rowData, rowIndex, columnIndex, tableView,
						column.getName(), rowSelected, rowHighlighted,
						rowDisabled);
		...
		if (showVerticalGridLines) {
			...
			for (int columnIndex = 0, columnCount = columns.getLength(); columnIndex < columnCount; columnIndex++) {
				...
				if (columnIndex < columnCount - 1
						|| includeTrailingVerticalGridLine) {
					GraphicsUtilities.drawLine(graphics, columnX, 0, height,
		...
		if (showHorizontalGridLines) {
			...
			for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
				...
				if (rowIndex < rowCount - 1
					...
					GraphicsUtilities.drawLine(graphics, 0, gridY, width,
				...
				for (int columnIndex = 0, columnCount = columns.getLength(); columnIndex < columnCount; columnIndex++) {
					...
					if (sortDirection != null) {
						for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
							...
							if (rowIndex < rowCount - 1
									|| includeTrailingHorizontalGridLine) {
								GraphicsUtilities.drawLine(graphics, columnX,
										gridY, columnWidth,
										Orientation.HORIZONTAL);
							}
						}
	...
	protected int getVariableRowHeight(int rowIndex,
		...
		for (int i = 0, n = columns.getLength(); i < n; i++) {
			...
			rowHeight = Math.max(rowHeight, cellRenderer
					.getPreferredHeight(columnWidthsArgument.get(i)));
		}
	...
	public Bounds getRowBounds(int rowIndex) {
		return new Bounds(0, getRowY(rowIndex), getWidth(),
	...
	public Bounds getColumnBounds(int columnIndex) {
		...
		return new Bounds(columnX, 0, columnWidths.get(columnIndex),
				getHeight());
	}
	...
	public Bounds getCellBounds(int rowIndex, int columnIndex) {
		...
		return new Bounds(cellX, rowIndex * (rowHeight + 1),
	...
	public static ArrayList<Integer> getColumnWidths(TableView tableView,
		...
		for (int i = 0; i < n; i++) {
			...
			if (column.isRelative()) {
				int columnWidth = (int) Math
						.round((double) (column.getWidth() * variableWidth)
								/ (double) relativeWidth);
				columnWidths.update(i, Math.min(
						Math.max(columnWidth, column.getMinimumWidth()),
						column.getMaximumWidth()));
			}
		}
	...
	public final void setSelectionBackgroundColor(
		...
		setSelectionBackgroundColor(GraphicsUtilities
				.decodeColor(selectionBackgroundColor));
	}
	...
	public final void setInactiveSelectionColor(String inactiveSelectionColor) {
		...
		setInactiveSelectionColor(GraphicsUtilities
				.decodeColor(inactiveSelectionColor));
	}
	...
	public final void setInactiveSelectionBackgroundColor(
		...
		setInactiveSelectionBackgroundColor(GraphicsUtilities
				.decodeColor(inactiveSelectionBackgroundColor));
	}

	public final void setInactiveSelectionBackgroundColor(
			int inactiveSelectionBackgroundColor) {
		TerraTheme theme = (TerraTheme) Theme.getTheme();
		setInactiveSelectionBackgroundColor(theme
	...
	public final void setHighlightBackgroundColor(
		...
		setHighlightBackgroundColor(GraphicsUtilities
				.decodeColor(highlightBackgroundColor));
	}
	...
	public final void setAlternateRowBackgroundColor(
			String alternateRowBackgroundColor) {
		if (alternateRowBackgroundColor == null) {
			throw new IllegalArgumentException(
					"alternateRowBackgroundColor is null.");
		}

		setAlternateRowBackgroundColor(GraphicsUtilities
				.decodeColor(alternateRowBackgroundColor));
	}

	public final void setAlternateRowBackgroundColor(
			int alternateRowBackgroundColor) {
		TerraTheme theme = (TerraTheme) Theme.getTheme();
		setAlternateRowBackgroundColor(theme
				.getColor(alternateRowBackgroundColor));
	}
	...
	public final void setColumnSelectionColor(String columnSelectionColor) {
		...
		setColumnSelectionColor(GraphicsUtilities
				.decodeColor(columnSelectionColor));
	}
	...
	public void setColumnSelectionHorizontalGridColor(
		...
		if (columnSelectionHorizontalGridColor == null) {
			throw new IllegalArgumentException(
					"columnSelectionHorizontalGridColor is null.");
		}
	...
	public final void setColumnSelectionHorizontalGridColor(
		...
		setColumnSelectionHorizontalGridColor(GraphicsUtilities
				.decodeColor(columnSelectionHorizontalGridColor));
	}
	...
	public final void setHorizontalGridColor(String horizontalGridColor) {
		...
		setHorizontalGridColor(GraphicsUtilities
				.decodeColor(horizontalGridColor));
	}
	...
	public void setIncludeTrailingVerticalGridLine(
			boolean includeTrailingVerticalGridLine) {
	...
	public void setIncludeTrailingHorizontalGridLine(
			boolean includeTrailingHorizontalGridLine) {
	...
	public boolean mouseDown(Component component, Mouse.Button button, int x,
		...
		if (rowIndex >= 0 && !tableView.isRowDisabled(rowIndex)) {
			...
			if (button == Mouse.Button.LEFT) {
				Keyboard.Modifier commandModifier = Platform
						.getCommandModifier();

				if (Keyboard.isPressed(Keyboard.Modifier.SHIFT)
						&& selectMode == TableView.SelectMode.MULTI) {
					Filter<?> disabledRowFilter = tableView
					...
					if (disabledRowFilter == null) {
						...
						if (startIndex == -1) {
							...
							Span selectedRange = (rowIndex > startIndex) ? new Span(
									startIndex, rowIndex) : new Span(rowIndex,
									endIndex);
	...
	public boolean mouseUp(Component component, Mouse.Button button, int x,
		...
		TableView tableView = (TableView) getComponent();
		if (selectIndex != -1
				&& tableView.getFirstSelectedIndex() != tableView
