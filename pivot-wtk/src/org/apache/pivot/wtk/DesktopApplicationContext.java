...
public final class DesktopApplicationContext extends ApplicationContext {
	...
	private static class DesktopDisplayHost extends DisplayHost {
		...
		public DesktopDisplayHost() {
			...
			display.getContainerListeners().add(
					new ContainerListener.Adapter() {
						@Override
						...
						public void componentsRemoved(Container container,
								int index, Sequence<Component> removed) {
		...
		private void updateFrameTitleBar() {
			...
			if (n == 0) {
				...
				Window topWindow = (Window) display
						.get(display.getLength() - 1);
			...
			if (rootOwnerLocal != previousRootOwner) {
				if (previousRootOwner != null) {
					previousRootOwner.getWindowListeners().remove(
							this.rootOwnerListener);
				}

				if (rootOwnerLocal != null) {
					rootOwnerLocal.getWindowListeners().add(
							this.rootOwnerListener);
				}
			...
			if (this.updateHostWindowTitleBarCallback == null) {
				this.updateHostWindowTitleBarCallback = new Runnable() {
					@Override
					public void run() {
						java.awt.Window hostWindow = getDisplay()
								.getHostWindow();

						if (DesktopDisplayHost.this.rootOwner == null) {
							((TitledWindow) hostWindow)
									.setTitle(DEFAULT_HOST_WINDOW_TITLE);
							hostWindow.setIconImage(null);
						} else {
							((TitledWindow) hostWindow)
									.setTitle(DesktopDisplayHost.this.rootOwner
											.getTitle());

							java.util.ArrayList<BufferedImage> iconImages = new java.util.ArrayList<BufferedImage>();
							for (Image icon : DesktopDisplayHost.this.rootOwner
									.getIcons()) {
								if (icon instanceof Picture) {
									iconImages.add(((Picture) icon)
											.getBufferedImage());
								}
							}
	...
	/**
	 * Returns an {@link EventEnabledFrame} if the java version is greater than
	 * or equal to 7. The returned frame is capable of painting its own title
	 * bar so that move events on the frame (normally not able to be received),
	 * can be dispatched.
	 * 
	...
	// Native host frame
	// David Ray : Forked change of access level (from private to protected) to
	// this class and its extention
	// to javax.swing.JFrame from java.awt.Frame
	public static class HostFrame extends java.awt.Frame implements
			TitledWindow {
	...
	private static class HostDialog extends java.awt.Frame implements
			TitledWindow {
		private static final long serialVersionUID = 5340356674429280196L;
	 ...
	 * 
	 * @param optional
	 *            If <tt>true</tt>, shutdown is optional and may be cancelled.
	 *            If <tt>false</tt>, shutdown cannot be cancelled.
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println("Unable to retrieve startup preferences: "
					+ exception);
		}
				...
				if (property.length == 2) {
					...
					try {
						...
						System.err.println(String.format(
								INVALID_PROPERTY_VALUE_MESSAGE, value, key));
					}
				} else {
					System.err.println(String.format(
							INVALID_PROPERTY_FORMAT_MESSAGE, arg));
				}
			} else {
				System.err.println(String.format(
						INVALID_PROPERTY_FORMAT_MESSAGE, arg));
		...
		if (application != null) {
			...
			boolean visible = (!preserveSplashScreen || SplashScreen
					.getSplashScreen() == null);
	 ...
	 * {@link #isRunningVersion7OrGreater()} to determine if it is safe to
	 * create a custom barchart frame.
	 * 
	...
	/**
	 * Returns a flag indicating whether the custom frame property was set to
	...
	private static void initializeOSExtensions() {
		...
		if (osName.toLowerCase(Locale.ENGLISH).startsWith("mac os x")) {
			try {
				...
				Class<?> eawtApplicationClass = Class
						.forName("com.apple.eawt.Application");
				Class<?> eawtApplicationListenerClass = Class
						.forName("com.apple.eawt.ApplicationListener");
				Class<?> eawtApplicationEventClass = Class
						.forName("com.apple.eawt.ApplicationEvent");

				Method setEnabledAboutMenuMethod = eawtApplicationClass
						.getMethod("setEnabledAboutMenu",
								new Class<?>[] { Boolean.TYPE });

				Method addApplicationListenerMethod = eawtApplicationClass
						.getMethod("addApplicationListener",
								new Class<?>[] { eawtApplicationListenerClass });

				final Method setHandledMethod = eawtApplicationEventClass
	...
	private static void setFullScreen(boolean fullScreen, boolean visible) {
		GraphicsDevice graphicsDevice = windowedHostFrame
	 ...
	 * This will cause a visible {@link SplashScreen} to be closed.<br/>
	 * It is intended to be called one time when the Pivot application has
	 * initialized its UI and the SplashScreen is ready to be dismissed, but can
	 * be safely called regardless of whether there is now, or used to be, a
	 * visible SplashScreen.
	 * 
	 * @param display
	 *            Display to make visible
	...
	public static void replaceSplashScreen(Display display) {
		...
		GraphicsDevice device = windowedHostFrame.getGraphicsConfiguration()
				.getDevice();
	...
	public static Display createDisplay(int width, int height, int x, int y,
			boolean modal, boolean resizable, boolean undecorated,
			java.awt.Window owner, DisplayListener displayCloseListener) {
		if (isFullScreen()) {
			throw new IllegalStateException();
		}

		final HostDialog hostDialog = new HostDialog(owner, modal,
				displayCloseListener);
