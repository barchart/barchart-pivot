...
public class TablePaneSkin extends ContainerSkin implements TablePane.Skin,
				...
				if (component != null && component.isVisible()) {
					...
					if (columnSpan > 1) {
						...
						if (spannedRelativeWeight > 0
							...
							int rowHeight = row.isRelative() ? -1 : row
									.getHeight();
							int componentPreferredWidth = component
					...
					if (rowSpan > 1) {
						...
						if (spannedRelativeWeight > 0
							...
							int componentPreferredHeight = component
									.getPreferredHeight(columnWidthsLocal[j]);
	...
	public int getBaseline(int width, int height) {
		...
		for (int i = 0; i < rowCount && baseline == -1; i++) {
			...
			for (int j = 0, n = row.getLength(); j < n && j < columnCount
				...
				if (component != null && component.isVisible()) {
					int columnSpan = Math
							.min(TablePane.getColumnSpan(component),
									columnCount - j);
					...
					int rowSpan = Math.min(TablePane.getRowSpan(component),
							rowCount - i);
		...
		for (int i = 0; i < rowCount; i++) {
			...
			for (int j = 0, n = row.getLength(); j < n && j < columnCount; j++) {
				...
				if (child != null && child.isVisible()) {
					...
					child.setSize(Math.max(childWidth, 0),
	...
	public void paint(Graphics2D graphics) {
		...
		if ((showHorizontalGridLines && verticalSpacing > 0)
			...
			if (showVerticalGridLines && horizontalSpacing > 0
					&& columnCount > 1) {
	 ...
	 *            Any of the {@linkplain GraphicsUtilities#decodeColor color
			...
			if (isRowVisible(i)) {
				...
				if (row.isRelative()) {
					...
					if (rowHeight < 0) {
						...
						rowHeight = getPreferredRowHeight(i,
				...
				if (component != null && component.isVisible()) {
					...
					if (rowSpan > 1) {
						...
						if (adjustCells && spannedDefaultHeightCellCount > 0) {
							int componentPreferredHeight = component
									.getPreferredHeight(columnWidthsArgument[j]);
	...
	public int getColumnAt(int x) {
		...
		for (int j = 0, columnX = padding.left; columnX <= x
				&& j < columnWidths.length; j++) {
			int columnWidth = columnWidths[j];
