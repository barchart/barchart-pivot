...
public class TableViewRowEditor extends Window implements TableView.RowEditor {
	...
	private ScrollPane scrollPane = new ScrollPane(
			ScrollPane.ScrollBarPolicy.NEVER, ScrollPane.ScrollBarPolicy.FILL);
	...
	@Override
	public void beginEdit(TableView tableViewArgument, int rowIndexArgument,
		...
		TableView.ColumnSequence tableViewColumns = tableViewArgument
				.getColumns();
		...
		for (int i = 0, n = tableViewColumns.getLength(); i < n; i++) {
			...
			tablePaneColumn
					.setWidth(tableViewArgument.getColumnBounds(i).width);
		...
		tableViewArgument.scrollAreaToVisible(rowBounds.x, rowBounds.y,
				rowBounds.width, rowBounds.height);

		// Constrain the bounds by what is visible through viewport ancestors
		rowBounds = tableViewArgument.getVisibleArea(rowBounds);
		Point location = tableViewArgument.mapPointToAncestor(
		...
		setLocation(location.x, location.y
				+ (rowBounds.height - getPreferredHeight(-1)) / 2);
